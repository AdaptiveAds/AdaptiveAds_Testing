...
namespace AdaptiveAds_TestFramework.PageFrameworks
{
    ...
    public static class LoginPage
    {
        ...
        {
            var errormessage = Driver.Instance.FindElement(By.ClassName(ConfigData.loginButtonClass));

            //
            if ((errormessage != null) == IsDiplayed) return;

            throw new Exception(string.Format("The error message visibility did not match that expected. "+
                                              "Expected {0} actual {1}", IsDiplayed, (errormessage != null)),null);
